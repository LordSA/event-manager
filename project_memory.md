...
* **Google Calendar Booking Engine:** `./app/components/GoogleCalendarView.tsx` supports Month, Week, Day, and Grid view modes. Features:
  ...
  - **Grid View Chronological Month Grouping & 6-Event Pagination:** Grid View limits initial visible cards to 6 per page (`EVENTS_PER_PAGE = 6`) with a glassmorphic pagination toolbar (`Prev`, `1`, `2`..., `Next`, `Showing 1–6 of N events`). Events remain sorted by start date and grouped into Month section headers. Modal popups use React `createPortal(..., document.body)` for zero-flicker screen centering.
