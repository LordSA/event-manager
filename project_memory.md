...
* **Realtime Events Hook & Poster Sync:** `./lib/hooks/useRealtimeEvents.ts` maps `poster_url`, `venue`, `perks`, and `system_prompt` from Supabase into `EventItemData`. `./app/admin/events/page.tsx` saves `poster_url` in Supabase & provides a live modal thumbnail preview. `./app/events/[id]/page.tsx` features an intelligent poster fallback card that prevents blank black boxes when no image is uploaded.
* **Public Summarizers & Dynamic Detail Page:** `./lib/summary.ts` (generates 2-line cards for directory & calendar popovers). `./app/events/[id]/page.tsx` fetches events by ID or slug via `.or('id.eq.X,slug.eq.X')`, checks `poster_url`, `image`, and `image_url` properties, and features a reset `imgError` state on route changes.
