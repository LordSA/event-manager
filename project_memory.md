...
* **Google Calendar Booking Engine:** `./app/components/GoogleCalendarView.tsx` supports Month, Week, Day, and Grid view modes. Features:
  ...
  - **Grid View Card Lift & React Portal Body Mounting:** Selected/hovered cards in Grid View elevate smoothly above surrounding cards (`hover:-translate-y-1.5 z-10 hover:shadow-2xl`). Modal popups use React `createPortal(..., document.body)` with `fixed inset-0 z-[99999] flex items-center justify-center` and backdrop click dismiss, guaranteeing popups mount directly to `document.body` and center 100% perfectly on screen with zero flickering or scroll offset bugs.
