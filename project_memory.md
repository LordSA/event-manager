...
* **Realtime Events Hook & Poster Sync:** `./lib/hooks/useRealtimeEvents.ts` maps `poster_url`, `venue`, `perks`, and `system_prompt` from Supabase into `EventItemData`. `./app/admin/events/page.tsx` features an adaptive payload retry handler that strips un-cached columns (`perks`) if not present in the Supabase schema cache and saves `poster_url` reliably to the database. `./app/events/[id]/page.tsx` renders purely the poster image with default fallback to `./public/images/poster.webp`.
* **Public Summarizers & Media Artwork Card:** `./lib/summary.ts` (generates 2-line cards for directory & calendar popovers). `./app/events/[id]/page.tsx` features a non-repetitive Campus Media Artwork Card with verified badge and pulse emblem when custom posters are not uploaded.
