...
* **Realtime Events Hook & Poster Sync:** `./lib/hooks/useRealtimeEvents.ts` maps `poster_url`, `venue`, `perks`, `slug`, and `system_prompt` from Supabase into `EventItemData`. `./app/admin/events/page.tsx` generates clean title slugs (`slug`) and saves `poster_url`. `./app/events/[id]/page.tsx` features `aspect-[3/4]` poster frames and separates UUID vs non-UUID slug query paths to prevent Postgres syntax errors.
* **Public Summarizers & 3-Line Description Overview:** `./lib/summary.ts` (generates 2-line cards for directory & calendar popovers). `refactorDescription3Lines` in `./app/events/[id]/page.tsx` limits the event summary to 3 concise lines (`line-clamp-3`), seamlessly connecting to the Ask Assistant drawer prompt.
