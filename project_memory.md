...
* **Google Calendar Booking Engine:** `./app/components/GoogleCalendarView.tsx` supports Month, Week, Day, and Grid view modes. Features:
  ...
  - **Grid View Chronological Month Grouping:** Grid View sorts events by start date (`a.date - b.date`) and groups them into Month & Year section headers (e.g. "August 2026", "September 2026") with event count badges, aligning Grid View directly with calendar chronology. Selected/hovered cards elevate smoothly (`hover:-translate-y-1.5 z-10 hover:shadow-2xl`). Modal popups use React `createPortal(..., document.body)` for zero-flicker screen centering.
