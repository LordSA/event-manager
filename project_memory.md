...
* **Realtime Events Hook & Poster Sync:** `./lib/hooks/useRealtimeEvents.ts` maps `poster_url`, `venue`, `perks`, `slug`, and `system_prompt` from Supabase into `EventItemData`. `./app/admin/events/page.tsx` generates clean title slugs (`slug`) and saves `poster_url`. `./app/page.tsx`, `./app/community/[id]/page.tsx`, and `./app/events/[id]/page.tsx` feature `aspect-[3/4]` poster frames and full title slug URL routing across all event cards.
