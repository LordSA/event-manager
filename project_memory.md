...
* **Realtime Events Hook & Navigation Sync:** `./lib/hooks/useRealtimeEvents.ts` maps `poster_url`, `venue`, `perks`, `slug`, and `system_prompt` from Supabase into `EventItemData`. `./app/page.tsx` links community cards to `/community/${c.slug || c.id}`. `./app/events/[id]/page.tsx` features smart referrer-aware back navigation (`router.back()`) dynamically labeled `Back to Home`, `Back to Community`, or `Back to All Events`. `./app/community/[id]/page.tsx` separates UUID vs slug queries.
