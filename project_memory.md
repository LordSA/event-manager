...
* **Event Format Selector & Categorization:** In `./app/admin/events/page.tsx`, slot booking modal includes an **Event Format / Mode** selector (`Offline`, `Online`, `Hybrid`) with `MapPin`, `Globe`, and `Zap` icons. Format is stored as part of the venue payload (`Offline • ...`, `Online • ...`, `Hybrid • ...`) and rendered with styled format badges on `./app/events/[id]/page.tsx`.
