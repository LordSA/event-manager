...
* **Google Calendar Booking Engine:** `./app/components/GoogleCalendarView.tsx` supports Month, Week, Day, and Grid view modes. Features:
  ...
  - **Day Cell Click Navigation:** Clicking any day cell in Month View or date header in Week View directly navigates to **Day Mode** (`viewMode = 'day'`) for that date. In Day Mode, admins can view all events on that timeline and click "+ Book Slot" in the toolbar or click any time slot row to open slot booking.
  - **Grid View Calendar-Projected Month Sync & 6-Event Pagination:** Grid View strictly displays events for the month and year selected in the top calendar header (`currentYear`, `currentMonth`). Redundant inner month headers are removed for a clean 3-column layout. 6-event pagination operates on the active projected month's events, automatically resetting to Page 1 when changing projected months or filters. Modal popups use React `createPortal(..., document.body)` for zero-flicker screen centering.
* **Glassmorphism Mobile Navigation:** `./app/components/AdminSidebar.tsx` renders a desktop sticky sidebar (`md:flex`) and a floating glassmorphic bottom navigation bar (`md:hidden fixed bottom-2.5 bg-[#0f121d]/85 backdrop-blur-2xl border-[#1e2436]/90`) with fluid typography (`shortLabel` fallbacks, single-line truncation `text-[9px] sm:text-[10px]`) optimized for all mobile screen sizes (320px–430px+), accompanied by a clean top header and `pb-28 md:pb-8` main view padding.
* **Closed Slot View Privacy Guard & Cross-Community RBAC:** In `./app/components/GoogleCalendarView.tsx` and `./app/admin/events/page.tsx`, public event page links (`/events/[id]`) are strictly hidden for draft (`closed`) slots, and edit/delete controls are strictly restricted to events belonging to the active user's own community (`isOwnCommunity` check). Non-superadmin leads cannot modify or delete events owned by other communities.
* **Event Format Selector & Categorization:** In `./app/admin/events/page.tsx`, slot booking modal includes an **Event Format / Mode** selector (`Offline`, `Online`) with `MapPin` and `Globe` icons. Format is stored as part of the venue payload (`Offline • ...`, `Online • ...`) and rendered with styled format badges on `./app/events/[id]/page.tsx`.
...
* **Bug Reporting & Support Portal & SMTP Email Dispatch:**
  - `./app/support/page.tsx`: Public bug reporting and support page asking for Name, Email, Phone, Issue Founded description, Screenshot/Proof upload or URL, and Suggestions/Feedback. Calls `./app/api/support/route.ts` which inserts into Supabase `support_tickets` table and dispatches an HTML email with screenshot attachment via `./lib/email.ts` to `room2homies@gmail.com` using SMTP (`smtp.gmail.com:465`). Linked from `./app/components/Footer.tsx`.
  - `./app/admin/support/page.tsx`: Superadmin receiver portal (`/admin/support`) restricted to `dev` and `admin` roles. Features ticket stats, status filters (`Open`, `In Progress`, `Resolved`, `Closed`), search query filter, image zoom modal via React `createPortal(..., document.body)`, real-time status updates, and ticket deletion controls. Linked in `./app/components/AdminSidebar.tsx`.
* **Poster Upload Dual Storage Pipeline:** `./lib/upload.ts` converts images to WebP and attempts Vercel Blob API (`/api/upload`), with an automatic fallback to Supabase Storage `posters` bucket for 100% upload reliability.
