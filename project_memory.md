...
* **Bug Reporting & Support Portal & SMTP Email Dispatch:**
  - `./app/support/page.tsx`: Public bug reporting and support page asking for Name, Email, Phone, Issue Founded description, Screenshot/Proof upload or URL, and Suggestions/Feedback. Calls `./app/api/support/route.ts` which inserts into Supabase `support_tickets` table, dynamically queries Supabase `profiles` for all superadmin emails (`role IN ('dev', 'admin')`), and dispatches an HTML email with screenshot attachment via `./lib/email.ts` using SMTP (`smtp.gmail.com:465`). Linked from `./app/components/Footer.tsx`.
