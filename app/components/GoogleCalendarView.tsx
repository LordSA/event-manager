...
export default function GoogleCalendarView({ events, communities, isManagerView = false }: GoogleCalendarViewProps) {
  ...
  return (
    <div className="bg-[#121212] border border-neutral-800 rounded-xl overflow-hidden shadow-lg flex flex-col">
      {/* Calendar Header & View Selector */}
      <div className="p-4 border-b border-neutral-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-[#171717]">
        <div className="flex items-center space-x-3">
          <button
            onClick={handleToday}
            className="px-3 py-1.5 rounded-lg bg-neutral-800 border border-neutral-700 text-xs font-medium text-white hover:bg-neutral-700 transition-colors"
          >
            Today
          </button>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="px-3 py-1.5 bg-neutral-800 border border-neutral-700 rounded-lg text-xs font-medium text-white focus:outline-none focus:border-neutral-500"
          >
      ...
      {viewMode === 'month' && (
        <div className="flex-1 flex flex-col min-h-[600px]">
          {/* Day Headers */}
          <div className="grid grid-cols-7 border-b border-neutral-800 bg-[#141414] text-center text-xs font-semibold text-neutral-400 py-2.5">
            {daysOfWeek.map((day) => (
              <div key={day}>{day}</div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 flex-1 auto-rows-fr bg-[#0a0a0a]">
            {Array.from({ length: startDay }).map((_, idx) => (
              <div key={`empty-${idx}`} className="border-r border-b border-neutral-800/60 bg-[#0d0d0d]/40 min-h-[110px]" />
            ))}
            ...
            {Array.from({ length: totalDays }).map((_, idx) => {
              ...
              return (
                <div
                  key={`day-${dayNum}`}
                  className="border-r border-b border-neutral-800 p-2 min-h-[110px] flex flex-col justify-start hover:bg-neutral-900/30 transition-colors"
                >
                    ...
                    {dayEvents.map((evt) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full text-left px-2 py-1 rounded bg-neutral-800 hover:bg-neutral-700 border border-neutral-700 transition-colors block group"
                        >
                          <div className="text-[11px] font-semibold text-white truncate leading-tight group-hover:text-neutral-200">
                            {evt.title}
                          </div>
                          <div className="text-[9px] text-neutral-400 truncate flex items-center justify-between mt-0.5">
      ...
      {(viewMode === 'week' || viewMode === 'day') && (
        <div className="flex-1 flex flex-col min-h-[650px] bg-[#0a0a0a]">
          {/* Header Row */}
          <div className={`grid ${viewMode === 'week' ? 'grid-cols-8' : 'grid-cols-2'} border-b border-neutral-800 bg-[#141414] text-center text-xs font-semibold text-neutral-400 py-2.5`}>
            <div className="w-16 text-center text-neutral-500 font-mono text-[11px]">Time</div>
            {(viewMode === 'week' ? weekDays : [currentDate]).map((d) => (
              <div key={d.toISOString()} className="flex flex-col items-center">
                ...
                <span className={`text-xs font-bold mt-0.5 ${d.toDateString() === new Date().toDateString() ? 'text-white' : 'text-neutral-300'}`}>
                  {d.getDate()}
                </span>
              </div>
            ))}
          ...
          <div className="flex-1 overflow-y-auto max-h-[600px] relative">
            {hours.map((hour) => (
              <div
                key={hour}
                className={`grid ${viewMode === 'week' ? 'grid-cols-8' : 'grid-cols-2'} border-b border-neutral-800/60 min-h-[60px]`}
              >
                <div className="w-16 border-r border-neutral-800 text-[10px] text-neutral-500 font-mono p-2 text-right select-none">
                ...
                {(viewMode === 'week' ? weekDays : [currentDate]).map((d) => {
                  ...
                  return (
                    <div
                      key={d.toISOString()}
                      className="border-r border-neutral-800/40 p-1 relative min-h-[60px]"
                    >
                      {dayEvents.map((evt) => {
                        ...
                        return (
                          <div
                            ...
                            className="absolute left-1 right-1 top-1 z-10 p-2 rounded-lg bg-neutral-800 border border-neutral-600 hover:border-neutral-400 cursor-pointer shadow-md flex flex-col justify-between transition-all overflow-hidden"
                          >
                            <div>
                              <span className="text-[9px] font-mono text-neutral-300 block uppercase font-bold">
                                {evt.category}
                              </span>
                            ...
                            <div className="text-[9px] text-neutral-400 font-mono flex items-center justify-between border-t border-neutral-700/60 pt-1 mt-1">
      ...
      {activeModalEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-[#171717] border border-neutral-700 rounded-xl p-6 max-w-md w-full space-y-4 shadow-2xl relative text-white">
            <button
            ...
            <div className="space-y-1">
              ...
              <h3 className="text-xl font-bold text-white mt-2">{activeModalEvent.title}</h3>
              <p className="text-xs text-neutral-400">{activeModalEvent.community}</p>
            </div>

            {activeModalEvent.description && (
              <p className="text-xs text-neutral-300 leading-relaxed border-t border-b border-neutral-800 py-3">
                {activeModalEvent.description}
              </p>
            )}
            ...
            <div className="pt-2 flex justify-end">
              <Link
                ...
                className="px-4 py-2 bg-white text-black hover:bg-neutral-200 font-semibold text-xs rounded-lg flex items-center space-x-1.5 transition-colors"
              >
