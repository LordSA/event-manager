...
import { Calendar as CalendarIcon, Plus, Lock, CheckCircle2, Trash2, Edit3, ShieldAlert } from 'lucide-react';
import { events as initialEvents, communities } from '@/app/lib/data';
import { UserRole } from '@/types/database.types';

export default function EventBookingEnginePage() {
  const [eventList, setEventList] = useState(
    initialEvents.map((e) => ({ ...e, status: 'live' as 'closed' | 'live' }))
  );
  // Simulated current user role & community (Defaults to Manager for IEEE SB CEV)
  ...
  const handleToggleStatus = (id: string) => {
    setEventList(
      eventList.map((e) =>
        e.id === id ? { ...e, status: e.status === 'live' ? 'closed' : 'live' } : e
      )
    );
  };

  const handleDelete = (id: string, evtCommunity: string) => {
    // RBAC Enforced: Editors CANNOT delete events
    if (currentUserRole === 'editor') {
      alert('RBAC Violation: Editors are not permitted to delete events. Contact your Community Manager.');
      return;
    }
    ...
    if (currentUserRole === 'manager' && userCommunity.toLowerCase() !== evtCommunity.toLowerCase()) {
      alert('RBAC Violation: Managers can only delete events belonging to their own community.');
      return;
    }
